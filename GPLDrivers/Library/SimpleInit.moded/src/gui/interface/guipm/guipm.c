...
#ifdef ENABLE_GUI
#include"../../../../libs/util-linux/libfdisk/libfdisk.h"
#include"str.h"
#include"gui.h"
#include"guipm.h"
#include"logger.h"
...
static void block_slider_cb(lv_event_t*e){
	struct size_block*pi=e->user_data;
	if(pi->max_sec<=0||lv_slider_is_dragged(pi->slider))return; //orig
	//	if(pi->max_sec<=0||lv_slider_is_dragged(obj))return; //from other
